...
|--------|-------|-------------|------|--------------|----------|
| POST | /api/auth/register | Register new user (Organiser/Participant). | Public | { fullName, email, password, role } | 201 Created; 400 Bad Request; 409 Conflict |
| POST | /api/auth/login | Authenticate user and return token. | Public | { email, password } | 200 OK; 401 Unauthorized |
