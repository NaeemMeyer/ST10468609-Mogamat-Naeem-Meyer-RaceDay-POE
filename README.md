...
|-------------|-------|-------------|---------------|--------------|------------------|
| POST | /api/auth/register | Registers a new RaceDay user and creates either an Organiser or Participant profile. | Public | { fullName, email, password, role, profileDetails } | 201 Created; 400 Bad Request; 409 Conflict |
| POST | /api/auth/login | Authenticates a registered user and returns role info + access token. | Public | { email, password } | 200 OK; 401 Unauthorized |
